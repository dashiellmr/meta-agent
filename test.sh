...
echo ""
echo "--- Compound commands: both parts safe → allow ---"
check "git log | head"         allow "$(bash_cmd 'git log --oneline | head -20')"
check "npm ci && npm run build" allow "$(bash_cmd 'npm ci && npm run build')"
check "ls | grep foo"          allow "$(bash_cmd 'ls -la | grep foo')"
check "cat file | wc -l"       allow "$(bash_cmd 'cat package.json | wc -l')"

echo ""
echo "--- Compound commands: one unknown part → ask ---"
check "ls && git push"         ask "$(bash_cmd 'ls && git push origin main')"
check "npm ci && terraform"    ask "$(bash_cmd 'npm ci && terraform plan')"

echo ""
echo "--- Subshells → ask ---"
check "subshell \$()"          ask "$(bash_cmd 'echo $(git rev-parse HEAD)')"
check "backtick subshell"      ask "$(tool Bash "$(printf '{"command":"echo `date`"}')")"
